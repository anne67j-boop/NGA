...
import { Loader2, Sparkles, AlertCircle, Film, PlayCircle, MonitorPlay, Key, ShieldCheck } from 'lucide-react';

const InnovationStudio: React.FC = () => {
  ...
  const handleConnect = async () => {
    ...
    try {
      if ((window as any).aistudio) {
        ...
        const has = await (window as any).aistudio.hasSelectedApiKey();
        setHasApiKey(has);
        if (!has) setError("API Key selection was cancelled or failed.");
      } else {
        setError("AI Studio environment not detected.");
      }
    } catch (e: any) {
      console.error(e);
      setError(e.message || "Connection failed. Please ensure popups are allowed.");
    }
  };
  ...
  const handleGenerate = async () => {
    ...
    if (!hasApiKey) {
      setError('Secure API session required. Please connect your account first.');
      return;
    }
    ...
    try {
      // Re-instantiate client with the (now guaranteed) environment key
      const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
      
      setStatusMessage('Sending prompt to Generative Video model...');
      ...
      if (errorMessage.includes("Requested entity was not found") || errorMessage.includes("404")) {
        errorMessage = "Session expired. Please reconnect your API key.";
        setHasApiKey(false);
      }
    ...
    <div className="bg-slate-950 min-h-screen font-sans text-slate-200">
       ...
       <div className="bg-slate-900 py-12 border-b border-slate-800 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10">
          ...
          <h1 className="text-3xl md:text-5xl font-bold text-white mb-4">Innovation Studio</h1>
        ...
        {!hasApiKey ? (
          <div className="max-w-md mx-auto bg-slate-900 border border-slate-700 rounded-xl p-8 text-center shadow-2xl">
            ...
            <p className="text-slate-400 mb-8">
              To access the Generative Video model, you must verify your session with a valid Google Cloud Project billing key.
            </p>
            <button
            ...
            {error && (
              <p className="mt-4 text-red-400 text-sm bg-red-900/20 p-2 rounded border border-red-900/50">
                {error}
              </p>
            )}
              ...
              <div className="bg-slate-900 rounded-xl border border-slate-800 p-6 shadow-lg">
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <textarea
                      rows={6}
                      className="w-full rounded bg-slate-950 border-slate-700 text-white border p-3 focus:ring-1 focus:ring-emerald-500 outline-none text-sm leading-relaxed"
              ...
              <div className="bg-slate-900 rounded-xl border border-slate-800 p-1 h-full min-h-[400px] flex flex-col">
                ...
                {videoUrl && (
                  <div className="p-4 flex justify-between items-center bg-slate-900">
                    <span className="text-xs text-slate-500 font-mono">Vision-Preview • 1080p</span>
                    <a 
                      href={videoUrl} 
